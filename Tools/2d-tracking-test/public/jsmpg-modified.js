...
var jsmpeg = window.jsmpeg = function( url, opts ) {
	...
	this.noDraw = opts.noDraw || false;
	this.currentFrameNum = 1;
	this.externalLoadCallback = opts.onload || null;
	this.externalDecodeCallback = opts.ondecodeframe || null;
	this.externalFinishedCallback = opts.onfinished || null;
	this.externalOnNextFrame = opts.onnextframe || null;
...
jsmpeg.prototype.nextFrame = function() {
	...
	while(true) {
		...
		if( code == START_SEQUENCE ) {
			...
			if(this.externalOnNextFrame) this.externalOnNextFrame();
			return this.canvas;
		}
...
jsmpeg.prototype.createTexture = function(index, name, program) {
	var gl = this.gl;
...
jsmpeg.prototype.initWebGL = function() {
	...
	try {
		...
		var gl = this.gl = this.canvas.getContext('webgl', options) || this.canvas.getContext('experimental-webgl', options);
	} catch (e) {
		return false;
	}
	...
	gl.attachShader(this.maskProgram, this.compileShader(gl.VERTEX_SHADER, SHADER_VERTEX_IDENTITY));
	gl.attachShader(this.maskProgram, this.compileShader(gl.FRAGMENT_SHADER, SHADER_FRAGMENT_YCBCRTORGBA_MASK));
	gl.linkProgram(this.maskProgram);

	gl.useProgram(this.maskProgram);

	vertexAttr = gl.getAttribLocation(this.maskProgram, 'vertex');
	gl.enableVertexAttribArray(vertexAttr);
	gl.vertexAttribPointer(vertexAttr, 2, gl.FLOAT, false, 0, 0);

	// setup textures
	this.YTexture2 = this.createTexture(0, 'YTexture',this.maskProgram);
	this.CBTexture2 = this.createTexture(1, 'CBTexture',this.maskProgram);
	this.CRTexture2 = this.createTexture(2, 'CRTexture',this.maskProgram);*/
...
jsmpeg.prototype.renderMask = function( y, cr, cb ) {
	var gl = this.gl;
