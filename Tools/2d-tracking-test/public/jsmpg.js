...
var jsmpeg = window.jsmpeg = function( url, opts ) {
	...
	this.maskedtarget = opts.maskedtarget;
	this.mask = opts.mask;
...
jsmpeg.prototype.renderFrame2D = function() {
	...
	if (this.mask) {
		...
		var alphaData = this.canvasContext.getImageData(0, 0, this.width, this.height).data;
		var image = this.mask.canvasContext.getImageData(0, 0, this.width, this.height);
		//console.log(image);
		for (var i = 3, len = image.data.length; i < len; i = i + 4) {
			if (alphaData[i-1] > 230) {
				alphaData[i-1] = 255;
			}
		    image.data[i] =  alphaData[i-1]; //copies blue channel of BW mask into A channel of the image
		}

		this.maskedtarget.putImageData(image, 0, 0, 0, 0, this.width, this.height);
	}
	else if( !this.noDraw ) {
		this.canvasContext.putImageData(this.currentRGBA, 0, 0);
	}
